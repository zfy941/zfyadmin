...
layui.define(["element","jquery"],function(exports){
        ...
        for(var i=0;i<data.length;i++){
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                if(data[i].icon != undefined && data[i].icon != ''){
                	ulHtml += '<i class="menu-icon '+data[i].icon+'" data-icon="'+data[i].icon+'"></i>';
                }
                ...
                for(var j=0;j<data[i].children.length;j++){
                    ...
                    if(data[i].children[j].icon != undefined && data[i].children[j].icon != ''){
                		ulHtml += '<i class="menu-icon '+data[i].children[j].icon+'" data-icon="'+data[i].children[j].icon+'"></i>';
                    }
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="/index/main"><i class="menu-icon layui-icon" data-icon=""></i><cite>后台首页</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
	...
	Tab.prototype.tabAdd = function(_this){
		...
		if(_this.attr("target") == "_blank"){
			...
			var title = '';
			if(_this.find("i.menu-icon").attr("data-icon") != undefined){
					title += '<i class="menu-icon '+_this.find("i.menu-icon").attr("data-icon")+'"></i>';
			}
			//已打开的窗口中不存在
